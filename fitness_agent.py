...
class FitnessAgent:
    def __init__(self, openai_api_key: str, nut_api_key: str):
        ...
        self.agent = Agent(
            ...
            functions=[self.get_nutritional_info,self.get_exercise_info, self.calculate_bmr, self.calculate_tdee, self.calculate_ibw, self.calculate_bmi, self.calculate_calories_to_lose_weight]
        )
    ...
    def get_exercise_info(self, query: str) -> dict:
        """Fetch the nutritional information for a specific food item.

        :param query: The food item to get nutritional info for
        :return: The nutritional information of the food item
        """
        api_url = 'https://api.api-ninjas.com/v1/exercises?query={}'.format(query)
        response = requests.get(api_url, timeout=100, headers={'X-Api-Key': self.nut_api_key})
